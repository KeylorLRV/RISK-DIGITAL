...
public class Mapa : MonoBehaviour
{
    ...
    void Awake()
    {
        // Inicialización del singleton si lo tienes.
        if (instance == null)
        {
            instance = this;
    ...
    void Start()
    {
        Debug.Log("=== START MAPA (Multijugador - Preparación Mínima) ===");

        // 1. Solo preparar estructuras básicas (listas vacías, referencias).
        // NO llamar a InicializarMapa() aquí.
        if (territorios == null)
            territorios = new ListaArray<Territorio>(); // Inicializar lista vacía.

        if (continentes == null)
            continentes = new ListaArray<Continente>(); // Inicializar lista vacía.

        if (territorioHandlers == null)
            territorioHandlers = new ListaArray<GameObject>(); // Inicializar lista vacía.

        // 2. Opcional: Esperar un frame para asegurar que la escena esté lista (si usas corutinas).
        // Si los GameObjects del mapa se instancian dinámicamente, usa esto.
        

        Debug.Log("Mapa preparado mínimamente. Esperando inicio de partida para inicialización completa.");
        Debug.Log("=== FIN START MAPA ===");
    }
